...
type UserStore interface {
	...
	SetPassword(userId UserId, password string, checkPasswordReuse bool) error                       // This sets the password to a user account
	...
	ResetPasswordFinish(userId UserId, token string, password string, checkPasswordReuse bool) error // Check that token matches the last one generated by ResetPasswordStart, and if so, call SetPassword
...
func (x *dummyUserStore) SetPassword(userId UserId, password string, checkPasswordReuse bool) error {
	x.usersLock.Lock()
	defer x.usersLock.Unlock()
	if user, exists := x.users[userId]; exists && user.authUserType.CanSetPassword() {
		if checkPasswordReuse && user.password == password {
...
func (x *dummyUserStore) ResetPasswordFinish(userId UserId, token string, password string, checkPasswordReuse bool) error {
	x.usersLock.Lock()
	defer x.usersLock.Unlock()
	if user, exists := x.users[userId]; exists && !user.archived && user.authUserType.CanSetPassword() {
...
func (x *sanitizingUserStore) SetPassword(userId UserId, password string, checkPasswordReuse bool) error {
	password = cleanPassword(password)
	return x.backend.SetPassword(userId, password, checkPasswordReuse)
}
...
func (x *sanitizingUserStore) ResetPasswordFinish(userId UserId, token string, password string, checkPasswordReuse bool) error {
	password = cleanPassword(password)
	if len(password) == 0 {
		return ErrInvalidPassword
	}
	return x.backend.ResetPasswordFinish(userId, token, password, checkPasswordReuse)
}
