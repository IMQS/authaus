...
type UserStore interface {
	...
	SetConfig(passwordExpiry time.Duration, maxFailedLoginCount int64) error
...
func (x *dummyUserStore) SetConfig(passwordExpiry time.Duration, maxFailedLoginCount int64) error {
...
func (x *dummyUserStore) CreateIdentity(user *AuthUser, password string) (UserId, error) {
	...
	if userD == nil {
		...
		x.users[userId] = &dummyUser{userId, user.Email, user.Username, user.Firstname, user.Lastname, user.Mobilenumber, user.Telephonenumber, user.Remarks, user.Created, user.CreatedBy, user.Modified, user.ModifiedBy, password, "", false, user.Type, user.PasswordModifiedDate, user.FailedLoginCount}
		return userId, nil
	} else {
...
func (x *dummyUserStore) GetIdentities(getIdentitiesFlag GetIdentitiesFlag) ([]AuthUser, error) {
	...
	for _, v := range x.users {
		...
		list = append(list, AuthUser{v.userId, v.email, v.username, v.firstname, v.lastname, v.mobilenumber, v.telephonenumber, v.remarks, v.created, v.createdby, v.modified, v.modifiedby, v.authUserType, v.archived, v.passwordModifiedDate, v.failedLoginCount})
	}
	return list, nil
}
...
func (x *dummyUserStore) GetUserFromIdentity(identity string) (AuthUser, error) {
	...
	for _, v := range x.users {
		if CanonicalizeIdentity(v.email) == CanonicalizeIdentity(identity) && v.archived == false {
			return AuthUser{UserId: v.userId, Email: v.email, Username: v.username, Firstname: v.firstname, Lastname: v.lastname, Mobilenumber: v.mobilenumber, Type: v.authUserType, PasswordModifiedDate: v.passwordModifiedDate, FailedLoginCount: v.failedLoginCount}, nil
...
func (x *sanitizingUserStore) SetConfig(passwordExpiry time.Duration, maxFailedLoginCount int64) error {
	return x.backend.SetConfig(passwordExpiry, maxFailedLoginCount)
}
