...
type Authenticator interface {
	...
	ResetPasswordStart(identity string, expires time.Time) (string, error)    // Create a one-time token that can be used to reset the password with a subsequent call to ResetPasswordFinish
	ResetPasswordFinish(identity string, token string, password string) error // Check that token matches the last one generated by ResetPasswordStart, and if so, call SetPassword
...
func (x *dummyAuthenticator) ResetPasswordStart(identity string, expires time.Time) (string, error) {
	x.usersLock.Lock()
	defer x.usersLock.Unlock()
	if user, exists := x.users[CanonicalizeIdentity(identity)]; exists {
...
func (x *dummyAuthenticator) ResetPasswordFinish(identity string, token string, password string) error {
	x.usersLock.Lock()
	defer x.usersLock.Unlock()
	if user, exists := x.users[CanonicalizeIdentity(identity)]; exists {
...
func cleanIdentityPassword(identity, password string) (string, string) {
	return cleanIdentity(identity), strings.TrimSpace(password)
}
...
func (x *sanitizingAuthenticator) ResetPasswordStart(identity string, expires time.Time) (string, error) {
	identity = cleanIdentity(identity)
	if len(identity) == 0 {
		return "", ErrIdentityEmpty
	}
	return x.backend.ResetPasswordStart(identity, expires)
}

func (x *sanitizingAuthenticator) ResetPasswordFinish(identity string, token string, password string) error {
	identity, password = cleanIdentityPassword(identity, password)
	if len(identity) == 0 {
		return ErrIdentityEmpty
	}
	...
	return x.backend.ResetPasswordFinish(identity, token, password)
}
