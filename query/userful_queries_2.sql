...
where username ilike '%abc%'
limit 10

-- all users
SELECT userid FROM authuserstore WHERE (LOWER(email) = LOWER('abc@acme.com')
    OR LOWER(username) = lower('def@acme.com'))
...
WHERE --(LOWER(aus.email) = 'abc@acme.com' ) OR
      LOWER(aus.lastname) ilike '%acm%' --'.callaghan@westerncape.gov.za') AND (aus.archived = false OR aus.archived IS NULL)
